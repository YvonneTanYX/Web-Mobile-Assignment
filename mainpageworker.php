...
/* ── HISTORY TAB SYSTEM (shared across Feeding, Health, Vax, Incidents) ── */
.hist-block-header{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:16px;}
.hist-tabs{display:flex;align-items:center;gap:0;background:var(--bg);border:1.5px solid var(--border);border-radius:10px;padding:3px;}
.hist-tab{padding:6px 16px;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;border:none;background:transparent;color:var(--sub);font-family:inherit;transition:all 0.18s;white-space:nowrap;}
.hist-tab.active{background:var(--white);color:var(--header);box-shadow:0 1px 5px rgba(0,0,0,0.09);}
.hist-controls{display:flex;align-items:center;gap:7px;flex-wrap:wrap;}
.hist-search{display:flex;align-items:center;gap:6px;background:var(--white);border:1.5px solid var(--border);border-radius:9px;padding:7px 11px;min-width:180px;transition:border-color 0.2s;}
.hist-search:focus-within{border-color:var(--header);}
.hist-search input{border:none;outline:none;font-size:12px;color:var(--text);background:transparent;font-family:inherit;width:100%;}
.hist-search input::placeholder{color:#bbb;}
.hist-filter{border:1.5px solid var(--border);border-radius:9px;padding:7px 10px;font-size:12px;font-family:inherit;color:var(--text);background:var(--white);outline:none;cursor:pointer;transition:border-color 0.2s;}
.hist-filter:focus{border-color:var(--header);}
.hist-clear{padding:7px 13px;border-radius:9px;border:1.5px solid var(--border);background:var(--white);color:var(--sub);font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.18s;}
.hist-clear:hover{border-color:var(--header);color:var(--header);}
.hist-showing{font-size:11px;color:var(--sub);align-self:center;white-space:nowrap;}
...
@media(max-width:768px){
  ...
  .hist-block-header{flex-direction:column;align-items:flex-start;}
}
...
<div class="main-content">
  ...
  <div class="page-wrap">
    ...
    <!-- FEEDING: History with Recent/History tabs -->
    <div class="block">
      <div class="hist-block-header">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <div class="hist-tabs">
            <button class="hist-tab active" id="feedHistTab-recent" onclick="switchFeedHistTab('recent',this)">Recent (3 days)</button>
            <button class="hist-tab" id="feedHistTab-history" onclick="switchFeedHistTab('history',this)">History</button>
          </div>
          <span class="hist-showing" id="feedHistShowing"></span>
        </div>
        <div class="hist-controls">
          <div class="hist-search">
            <span class="iconify" data-icon="lucide:search" style="font-size:13px;color:#bbb;flex-shrink:0;"></span>
            <input type="text" id="feedHistSearch" placeholder="Search animal, food type..." oninput="applyFeedHistFilters()">
          </div>
          <input type="date" id="feedHistDate" class="hist-filter" title="Filter by date" onchange="applyFeedHistFilters()">
          <button class="hist-clear" onclick="clearFeedHistFilters()">Show All</button>
        </div>
      </div>
      <div class="table-wrap">
        <table class="data-table">
  ...
  <div class="page-wrap">
    ...
    <!-- HEALTH: Records with Recent/History tabs -->
    <div class="block">
      <div class="hist-block-header">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <div class="hist-tabs">
            <button class="hist-tab active" id="healthHistTab-recent" onclick="switchHealthHistTab('recent',this)">Recent (3 days)</button>
            <button class="hist-tab" id="healthHistTab-history" onclick="switchHealthHistTab('history',this)">History</button>
          </div>
          <span class="hist-showing" id="healthHistShowing"></span>
        </div>
        <div class="hist-controls">
          <div class="hist-search">
            <span class="iconify" data-icon="lucide:search" style="font-size:13px;color:#bbb;flex-shrink:0;"></span>
            <input type="text" id="healthHistSearch" placeholder="Search animal, event type..." oninput="applyHealthHistFilters()">
          </div>
          <select class="hist-filter" id="healthHistType" onchange="applyHealthHistFilters()">
            <option value="">All Types</option>
            <option>Routine Check</option>
            <option>Illness Observation</option>
            <option>Vet Visit</option>
            <option>Medication Given</option>
            <option>Post-Surgery Observation</option>
            <option>Injury Report</option>
          </select>
          <button class="hist-clear" onclick="clearHealthHistFilters()">Show All</button>
        </div>
      </div>
  ...
  <div class="page-wrap">
    ...
    <!-- VAX: Schedule with Recent/History tabs -->
    <div class="block">
      <div class="hist-block-header">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <div class="hist-tabs">
            <button class="hist-tab active" id="vaxHistTab-recent" onclick="switchVaxHistTab('recent',this)">Recent (3 days)</button>
            <button class="hist-tab" id="vaxHistTab-history" onclick="switchVaxHistTab('history',this)">History</button>
          </div>
          <span class="hist-showing" id="vaxHistShowing"></span>
        </div>
        <div class="hist-controls">
          <div class="hist-search">
            <span class="iconify" data-icon="lucide:search" style="font-size:13px;color:#bbb;flex-shrink:0;"></span>
            <input type="text" id="vaxHistSearch" placeholder="Search animal, vaccine..." oninput="applyVaxHistFilters()">
          </div>
          <select class="hist-filter" id="vaxHistVaccine" onchange="applyVaxHistFilters()">
            <option value="">All Vaccines</option>
            <option>Rabies Vaccine</option>
            <option>Distemper Vaccine</option>
            <option>Hepatitis Vaccine</option>
            <option>Tetanus Booster</option>
            <option>FMD Vaccine</option>
            <option>Other</option>
          </select>
          <button class="hist-clear" onclick="clearVaxHistFilters()">Show All</button>
        </div>
      </div>
      <div id="vaxList">
  ...
  <div class="page-wrap">
    ...
    <!-- INCIDENTS: History with Recent/History tabs + severity filter -->
    <div class="block">
      <div class="hist-block-header">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <div class="hist-tabs">
            <button class="hist-tab active" id="incHistTab-recent" onclick="switchIncHistTab('recent',this)">Recent (3 days)</button>
            <button class="hist-tab" id="incHistTab-history" onclick="switchIncHistTab('history',this)">History</button>
          </div>
          <span class="hist-showing" id="incHistShowing"></span>
        </div>
        <div class="hist-controls">
          <div class="hist-search">
            <span class="iconify" data-icon="lucide:search" style="font-size:13px;color:#bbb;flex-shrink:0;"></span>
            <input type="text" id="incHistSearch" placeholder="Search type, description..." oninput="applyIncHistFilters()">
          </div>
          <select class="hist-filter" id="incHistSeverity" onchange="applyIncHistFilters()">
            <option value="">All Severities</option>
            <option value="low">🟢 Low</option>
            <option value="medium">🟡 Medium</option>
            <option value="high">🔴 High</option>
          </select>
          <button class="hist-clear" onclick="clearIncHistFilters()">Show All</button>
        </div>
      </div>
...
setInterval(()=>{
  const now=new Date();
  document.getElementById('liveDate').textContent=now.toLocaleString('en-MY',{weekday:'long',year:'numeric',month:'long',day:'numeric',hour:'2-digit',minute:'2-digit'});
  if(currentWorker){document.getElementById('greetingText').textContent=getGreeting()+', '+currentWorker.username+' 👋';}
},1000);
...
function toggleNotifPanel() {
  ...
  closeAllDropdowns();
  if (!isOpen) { panel.classList.add('open'); renderNotifPanel(); }
}
...
function handleGlobalSearch(q){
  ...
  const results = [];
  const pageMatches = PAGES_INDEX.filter(p=>p.label.toLowerCase().includes(q)||p.sub.toLowerCase().includes(q));
  if(pageMatches.length){results.push({type:'category',label:'Pages'});pageMatches.forEach(p=>results.push({type:'page',...p}));}
  const animalMatches = animals.filter(a=>a.name.toLowerCase().includes(q)||a.species.toLowerCase().includes(q)||(a.zone||'').toLowerCase().includes(q)||a.status.toLowerCase().includes(q)||(a.notes||'').toLowerCase().includes(q));
  if(animalMatches.length){results.push({type:'category',label:'Animals'});animalMatches.slice(0,5).forEach(a=>results.push({type:'animal',...a}));}
  const taskMatches = tasks.filter(t=>t.name.toLowerCase().includes(q)||t.meta.toLowerCase().includes(q)||t.zone.toLowerCase().includes(q));
  if(taskMatches.length){results.push({type:'category',label:'Tasks'});taskMatches.slice(0,4).forEach(t=>results.push({type:'task',...t}));}
  const feedMatches = feedingLog.filter(r=>r.animal.toLowerCase().includes(q)||r.type.toLowerCase().includes(q)||(r.notes||'').toLowerCase().includes(q));
  if(feedMatches.length){results.push({type:'category',label:'Feeding Records'});feedMatches.slice(0,3).forEach(r=>results.push({type:'feed',...r}));}
  const incMatches = incidentLog.filter(r=>r.title.toLowerCase().includes(q)||r.desc.toLowerCase().includes(q));
  if(incMatches.length){results.push({type:'category',label:'Incidents'});incMatches.slice(0,3).forEach(r=>results.push({type:'incident',...r}));}
  if(!results.length){dd.innerHTML=`<div class="search-no-results">No results found for "<strong>${q}</strong>"</div>`;dd.classList.add('open');return;}
  dd.innerHTML = results.map(r=>{
    ...
    if(r.type==='animal'){const st={healthy:'Healthy',watch:'Under Observation',treatment:'Under Treatment'};return `<div class="search-result-item" onclick="searchGoToPage('pg-animals',1)"><div class="sri-icon">${r.emoji}</div><div><div class="sri-title">${r.name}</div><div class="sri-sub">${r.species} · ${st[r.status]}</div></div></div>`;}
...
function closeAllDropdowns(){closeSearchDropdown();document.getElementById('notifPanel').classList.remove('open');}
...
function buildEmojiPicker(){
  document.getElementById('emojiPicker').innerHTML=EMOJIS.map(e=>`<div class="emoji-opt${e===selEmoji?' sel':''}" onclick="pickEmoji(this,'${e}')">${e}</div>`).join('');
}
...
function renderAnimals(){
  ...
  if(!list.length){g.innerHTML=`<div class="empty-state"><div class="empty-icon">🐾</div><p>${animals.length?'No animals match your search.':'No animals yet. Click "Add Animal" to get started.'}</p></div>`;return;}
...
async function saveAnimal(){
  ...
  try{
    ...
    else{const newId=animals.length?Math.max(...animals.map(a=>a.id))+1:1;animals.push({id:newId,...d});showToast(`🐾 ${name} added!`);pushNotif('🐾','ni-green',`New animal added`,`${name} (${species}) has been added to the system.`);}
    LS.set('animals',animals);closeAnimalModal();renderAnimals();syncDashboard();populateDropdowns();
  }
}
...
async function confirmDelete(){
  ...
  try{
    ...
    LS.set('animals',animals);closeDeleteModal();renderAnimals();syncDashboard();populateDropdowns();
...
let feedingCount=0,healthCount=0,vaxCount=0,incidentCount=0;
...
async function loadFeedingLog(){
  ...
  applyFeedHistFilters(); // refresh history view too
}
...
// ═══ FEEDING HISTORY — Recent/History tabs ═══
let feedHistMode = 'recent'; // 'recent' | 'history'
let feedingHistory = [];

function switchFeedHistTab(mode, btn) {
  feedHistMode = mode;
  document.querySelectorAll('[id^="feedHistTab-"]').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  // Show date filter only in history mode
  document.getElementById('feedHistDate').style.display = mode === 'history' ? '' : 'none';
  applyFeedHistFilters();
}

async function loadFeedingHistory() {
  const dateFilter = feedHistMode === 'history' ? (document.getElementById('feedHistDate').value || '') : '';
  let url = 'api/feeding_worker.php?mode=' + (feedHistMode === 'recent' ? 'recent' : 'history');
...
async function applyFeedHistFilters() {
  await loadFeedingHistory();
  const q = (document.getElementById('feedHistSearch').value || '').toLowerCase().trim();
  let list = feedingHistory;
  if (q) list = list.filter(r =>
    (r.animal||'').toLowerCase().includes(q) ||
    (r.type||'').toLowerCase().includes(q) ||
    (r.notes||'').toLowerCase().includes(q) ||
    (r.consumed||'').toLowerCase().includes(q) ||
    (r.worker||'').toLowerCase().includes(q)
  );
  renderFeedingHistory(list);
  const showing = document.getElementById('feedHistShowing');
  showing.textContent = list.length
    ? `Showing ${list.length} record${list.length!==1?'s':''}` + (q ? ` for "${q}"` : '')
    : '';
}

function clearFeedHistFilters() {
  document.getElementById('feedHistSearch').value = '';
  document.getElementById('feedHistDate').value = '';
  applyFeedHistFilters();
}

function renderFeedingHistory(list) {
  const tbody = document.getElementById('feedingHistoryBody');
  if (!tbody) return;
  if (!list || !list.length) {
    tbody.innerHTML = `<tr class="table-empty-row"><td colspan="8">${
      feedHistMode === 'recent' ? 'No feeding records in the last 3 days.' : 'No feeding records found.'
    }</td></tr>`;
    return;
  }
  const bc = {'All Eaten':'b-green','Refused Food':'b-red','75% Eaten':'b-orange','50% Eaten':'b-orange','25% Eaten':'b-red'};
  tbody.innerHTML = list.map(r => `<tr>
    ...
    <td>${r.time||'—'}</td><td>${r.animal||'—'}</td><td>${r.type||'—'}</td>
    <td>${r.qty||'—'}</td>
...
async function loadHealthLog(){
  try{
    ...
    const data=await res.json();
    if(data.success){healthLog=data.records.map(r=>({...r,rawNextCheckup:r.rawNextCheckup||''}));LS.set('healthLog',healthLog);}
  }catch(e){healthLog=LS.get('healthLog',[]);}
  healthCount=healthLog.length;LS.set('healthCount',healthCount);
  applyHealthHistFilters();syncDashboard();
}

// ═══ HEALTH HISTORY — Recent/History tabs ═══
let healthHistMode = 'recent';

function switchHealthHistTab(mode, btn) {
  healthHistMode = mode;
  document.querySelectorAll('[id^="healthHistTab-"]').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  applyHealthHistFilters();
}

function applyHealthHistFilters() {
  const q = (document.getElementById('healthHistSearch').value || '').toLowerCase().trim();
  const typeFilter = (document.getElementById('healthHistType').value || '').toLowerCase();
  const threeDaysAgo = Date.now() - 3 * 24 * 60 * 60 * 1000;

  let list = healthLog;

  if (healthHistMode === 'recent') {
    // filter to records from last 3 days — use dateStr heuristic
    list = list.filter(r => {
      if (!r.dateStr) return true; // keep if unknown
      // Try to detect "Today", "Yesterday" or a date within 3 days
      const ds = r.dateStr.toLowerCase();
      if (ds.startsWith('today')) return true;
      if (ds.startsWith('yesterday')) return true;
      // fallback: keep all (server should already scope recent)
      return true;
    });
  }
  ...
  if (typeFilter) list = list.filter(r => (r.type||'').toLowerCase() === typeFilter);

  renderHealthRecords(list);
  const showing = document.getElementById('healthHistShowing');
  showing.textContent = list.length
    ? `Showing ${list.length} record${list.length!==1?'s':''}` + (q||typeFilter ? ' (filtered)' : '')
    : '';
}

function clearHealthHistFilters() {
  document.getElementById('healthHistSearch').value = '';
  document.getElementById('healthHistType').value = '';
  applyHealthHistFilters();
}
...
function renderHealthRecords(list){
  ...
  if(!list||!list.length){
    container.innerHTML=`<p class="placeholder-msg">${
      healthHistMode==='recent'?'No health records in the last 3 days.':'No health records found.'
    }</p>`;
    return;
  }
...
async function logHealth(){
  ...
  try{
    ...
    else{healthLog.unshift(entry);LS.set('healthLog',healthLog);applyHealthHistFilters();}
  }catch(e){healthLog.unshift(entry);LS.set('healthLog',healthLog);healthCount=healthLog.length;LS.set('healthCount',healthCount);applyHealthHistFilters();}
...
// ═══ VAX HISTORY — Recent/History tabs ═══
let vaxHistMode = 'recent';

function switchVaxHistTab(mode, btn) {
  vaxHistMode = mode;
  document.querySelectorAll('[id^="vaxHistTab-"]').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  applyVaxHistFilters();
}

function applyVaxHistFilters() {
  const q = (document.getElementById('vaxHistSearch').value || '').toLowerCase().trim();
  const vaccineFilter = (document.getElementById('vaxHistVaccine').value || '').toLowerCase();
  const threeDaysAgo = Date.now() - 3 * 24 * 60 * 60 * 1000;

  let list = vaxLog;

  if (vaxHistMode === 'recent') {
    // Filter to last 3 days by rawDate
    list = list.filter(r => {
      if (!r.rawDate) return true;
      return new Date(r.rawDate).getTime() >= threeDaysAgo;
    });
  }
...
async function logVaccination() {
  ...
  const payload = { animal_id: animal.id, vaccine_name: name, date_given: date, next_due_date: next, vet_name: vet };
  try {
    const res = await fetch('api/vaccinations_worker.php', { method: 'POST', credentials: 'include', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await res.json();
    if (data.success) {
...
let tasks=[],taskNextId=1,taskFilter='all',editingTaskId=null;
...
function renderTasks(){
  ...
  if(!filtered.length){
    tl.innerHTML=`<div class="empty-state" style="padding:30px 0;"><div class="empty-icon">📋</div><p>${taskFilter==='done'?'No completed tasks yet.':taskFilter==='inactive'?'No inactive tasks.':'No tasks match this filter.'}</p></div>`;
...
async function toggleTask(id){
  ...
  try{
    const res=await fetch('api/dailytask_worker.php',{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,done:!t.done})});
    const data=await res.json();
    if(data.success){if(!t.done)pushNotif('✅','ni-green','Task completed',`"${t.name}" marked as done.`);await loadTasksFromAPI();syncDashboard();}
    else showToast(data.message,'error');
  }catch(e){showToast('Network error','error');}
}
...
async function toggleTaskActive(id){
  ...
  try{
    const res=await fetch('api/dailytask_worker.php',{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,active:newActive,done:false})});
    const data=await res.json();
    if(data.success){showToast(newActive?'✅ Task activated!':'⏸️ Task deactivated.');await loadTasksFromAPI();syncDashboard();}
    else showToast(data.message,'error');
  }catch(e){showToast('Network error','error');}
}
...
async function saveTask(){
  ...
  const payload={name,meta:meta||'No details',zone,priority,active};
  let method='POST',url='api/dailytask_worker.php';
  if(editingTaskId){method='PUT';payload.id=editingTaskId;}
  try{
    const res=await fetch(url,{method,credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const data=await res.json();
    if(data.success){showToast(editingTaskId?'✏️ Task updated!':'📋 Task added!');await loadTasksFromAPI();closeModal('taskModal');syncDashboard();}
    else showToast(data.message,'error');
  }catch(e){showToast('Network error','error');}
}
...
async function deleteTask(id){
  ...
  try{
    const res=await fetch('api/dailytask_worker.php',{method:'DELETE',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
    const data=await res.json();
    if(data.success){showToast('🗑️ Task deleted.');await loadTasksFromAPI();syncDashboard();}
    else showToast(data.message,'error');
  }catch(e){showToast('Network error','error');}
}
...
// ═══ INCIDENT HISTORY — Recent/History tabs + severity filter ═══
let incHistMode = 'recent';
let incidentHistoryFull = []; // all raw incidents from API

function switchIncHistTab(mode, btn) {
  incHistMode = mode;
  document.querySelectorAll('[id^="incHistTab-"]').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  applyIncHistFilters();
}

function applyIncHistFilters() {
  const q = (document.getElementById('incHistSearch').value || '').toLowerCase().trim();
  const sevFilter = (document.getElementById('incHistSeverity').value || '').toLowerCase();
  const threeDaysAgo = Date.now() - 3 * 24 * 60 * 60 * 1000;

  let list = incidentHistoryFull.length ? incidentHistoryFull : incidentLog;

  if (incHistMode === 'recent') {
    list = list.filter(r => {
      if (r.rawTs) return r.rawTs >= threeDaysAgo;
      // fallback: show all if no rawTs
      return true;
    });
  }
  ...
  if (sevFilter) list = list.filter(r => (r.severity||'').toLowerCase() === sevFilter);

  renderIncidents(list);
  const showing = document.getElementById('incHistShowing');
  showing.textContent = list.length
    ? `Showing ${list.length} report${list.length!==1?'s':''}` + (q||sevFilter ? ' (filtered)' : '')
    : '';
}

function clearIncHistFilters() {
  document.getElementById('incHistSearch').value = '';
  document.getElementById('incHistSeverity').value = '';
  applyIncHistFilters();
}
...
async function loadIncidentsFromAPI() {
  try {
    ...
    const data = await res.json();
    if (data.success) {
      incidentHistoryFull = data.incidents.map(inc => ({
        ...
        rawTs: new Date(inc.reported_at).getTime(),
        id: inc.id
      }));
      incidentLog = incidentHistoryFull; // keep in sync for global search
...
async function submitIncident() {
  ...
  const payload = { incident_type: type, description: desc, animal_id: animalId, severity: selSev };
  try {
    const res = await fetch('api/incident_worker.php', { method: 'POST', credentials: 'include', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await res.json();
    if (data.success) {
...
async function saveEditFeeding(){
  ...
  r.type=document.getElementById('ef_type').value;r.qty=parseFloat(document.getElementById('ef_qty').value).toFixed(1);
  r.consumed=document.getElementById('ef_consumed').value;r.notes=document.getElementById('ef_notes').value.trim();
...
function editHealth(i){editHealthIdx=i;const r=healthLog[i];document.getElementById('eh_type').value=r.type;document.getElementById('eh_notes').value=r.notes||'';document.getElementById('eh_treatment').value=r.treatment||'';document.getElementById('eh_next_checkup').value=r.rawNextCheckup||'';document.getElementById('editHealthModal').classList.add('open');}
async function saveEditHealth(){
  const r=healthLog[editHealthIdx];
  r.type=document.getElementById('eh_type').value;r.notes=document.getElementById('eh_notes').value.trim();
  r.treatment=document.getElementById('eh_treatment').value.trim();
  const rawNext=document.getElementById('eh_next_checkup').value;
  r.rawNextCheckup=rawNext;r.next_checkup=rawNext?new Date(rawNext+'T00:00:00').toLocaleDateString('en-MY',{day:'numeric',month:'short',year:'numeric'}):'';
...
async function saveEditVax(){
  ...
  r.name=document.getElementById('ev_name').value;r.rawDate=document.getElementById('ev_date').value;
  r.rawNext=document.getElementById('ev_next').value;r.vet=document.getElementById('ev_vet').value.trim();
...
let editIncIdx=null;
function editIncident(i){editIncIdx=i;document.getElementById('ei_desc').value=incidentHistoryFull[i]?incidentHistoryFull[i].desc:incidentLog[i].desc;document.getElementById('editIncModal').classList.add('open');}
async function saveEditIncident(){
  const r=incidentHistoryFull[editIncIdx]||incidentLog[editIncIdx];
...
async function deleteIncident(i){
  ...
  const r=incidentHistoryFull[i]||incidentLog[i];
...
async function loadTasksFromAPI(){
  ...
  try{
    ...
    const data=await res.json();
    if(data.success){
      tasks=data.tasks.map(t=>({id:t.id,name:t.name,meta:t.meta,zone:t.zone,priority:t.priority,done:t.done,active:t.active,createdBy:t.created_by_name||''}));
      LS.set('tasks',tasks);renderTasks();syncDashboard();
    }
  }catch(e){console.warn(e);}
}

// ─── INIT: hide date filter on load (only visible in 'history' mode for feeding) ───
document.addEventListener('DOMContentLoaded', () => {
  const feedDateEl = document.getElementById('feedHistDate');
  if (feedDateEl) feedDateEl.style.display = 'none';
});

// ─── BOOT ───
_updateNotifBadge();
applyFeedHistFilters(); // initial load for feeding history
initSession();
</script>
</body>
</html>
